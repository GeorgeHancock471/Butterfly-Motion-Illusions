...
% NAME:  function for images, used to cycle masks

% Function:
% This script is used to cycle through target alpha images
...
function target = f_images_cycles(game,target,window)
     ...
     for(m = 1:game.nTargets) % Loop for each Target
            ...
            folderPaths = fullfile(game.folderPath,'Targets', char(target.TargetD(m)));
            ...
            imagePath = fullfile(game.folderPath,'Targets', char(target.TargetD(m)),folderNames(z))
            ...
            disp(imageFiles(1));
             ...
             for k = 1:length(imageFiles)
                 currentPath = fullfile(imageFiles(1).folder,"\",imageFiles(k).name);
                    ...
                    texture = Screen('MakeTexture', window, cat(3, currentAlpha, currentAlpha));
