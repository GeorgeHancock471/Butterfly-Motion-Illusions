...
% NAME:  function for targets, load Masks

% Function:
% This function will load the image masks for the different methods of rendering
...
function target = f_target_loadMasks(game,target,window)

    if(game.animationMethod=="Cycles")
    target = f_images_cycles_masks(game,target,window);
    end
    
    if(game.animationMethod=="Moth2D")
    target = f_images_moth2D_masks(game,target,window);
    end
    
    if(game.animationMethod=="Image")
    target = f_images_single_mask(game,target,window);
    end
