...
% Function:
% This script is used to load the image if the target is a single image.
...
function target = f_images_single(game,target,window)

    for(m = 1:game.nTargets) % Loop for each Target
   
    imagePath = fullfile(game.targets, char(target.TargetD(m)), 'Target.png');
    disp(imagePath);
    ...
    if(game.useHitbox)
    imagePath = fullfile(game.targets, char(target.TargetD(m)), 'Hitbox.png');
