...
resultsDir = fullfile(gamePath, 'Results');
if ~exist(resultsDir, 'dir')
    mkdir(resultsDir);
end

files = dir(fullfile(resultsDir,'*.csv'));   % all previous result files
playerNum = numel(files) + 1;                % next available ID

defaultPlayerID = sprintf('P%03d',playerNum);
...
mouse = f_game_UI (window,windowRect,game,10,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
if(mouse.x < windowRect(3)/2)
handedness = "left";
else
handedness = "right";
end
...
f_game_UI (window,windowRect,game,2,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
f_game_UI (window,windowRect,game,3,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
f_game_UI (window,windowRect,game,4,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
f_game_UI (window,windowRect,game,5,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
if(Tutorials)
f_game_UI (window,windowRect,game,6,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
end
end
...
for expS = 1:numel(TutorialA);
    nHits=0;
    trialN = 0;


exitLoops = false;
Tutorial = TutorialA{expS};
...
% Draw centered text
DrawFormattedText(window, 'Well done, you hit two butterflies! The butterflies will now go faster!', 'center', 'center', textColor);
...
if(~Tutorial)
if(Tutorials)
f_game_UI (window,windowRect,game,8,lines,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
end
% Loading
%....................................................................
...
% Loop Sequence
nRep = 3;
nPaths = 10;   % e.g. 30, change to 6 for testing
...
nButterflies = numel(butterflyIDs);

% Build repeated, randomized butterfly list
expandedButterflies = {};

while numel(expandedButterflies) < nPaths
    shuffledIDs = butterflyIDs(randperm(numel(butterflyIDs)));
    expandedButterflies = [expandedButterflies, shuffledIDs]; %#ok<AGROW>
end

% Trim to match number of flight paths
expandedButterflies = expandedButterflies(1:nPaths);
...
Butterfly_ID =  expandedButterflies{index};
Flight_Path =  flightPaths{index};


% Stimulus Path
% ...............
stimuliRoot = fullfile(gamePath, 'stimuli_new', Butterfly_ID, Flight_Path);
stimuliT = fullfile(stimuliRoot,'Textured_reduced_EB');
...
results.clickHit = 0; %Fill by game
...
SetMouse(windowRect(3)/2, windowRect(4)/2, 0);
dotSize=70;
dotColor=[1,1,1];
line1="";
   line2="";
...
hitboxD = fullfile(stimuliRoot,"Hitbox_Adjust",hitboxSlide);
...
% Valid pixels: inside circle and nonzero G & B
validMask = mask & (G ~= 0) & (B ~= 0);

% Compute squared distance to the clicked pixel
distSq = (X - imgX).^2 + (Y - imgY).^2;
distSq(~validMask) = inf;

if any(validMask(:))

    % Find closest valid pixel
    [~, idx] = min(distSq(:));
    [row, col] = ind2sub(size(distSq), idx);

    % Mean of R over valid pixels
    meanR = mean(R(validMask));

    % G and B from closest valid pixel
    meanG = G(row, col);
    meanB = B(row, col);
...
% Store trial results
Results(resultRow, :) = { ...
    playerID,...               % Which player played the game.
    handedness,...             % Which hand is dominant for the player?
    Tutorial,...               % Was the trial part of the tutorial?
    expS,...                  % Which phase of the experiment is it?
    ...
    results.click, ...         % Was there a click
...
pause(2);
    ...
    variableNames = { ...
               ...
               'Handedness',...        % Which hand is dominant for the player?
               'Tutorial', ...         % Was the trial part of the tutorial
               'Phase',...,            % Which phase of the experiment is it?
               'trialNumber', ...      % Trial number
               'butterfly', ...        % Butterfly
               'pathNumber', ...       % Flight Path
               'flipX', ...            % Flip Stimulus X
               'flipY', ...            % Flip Stimulus Y
               'click', ...            % Was there a click?
               'clickX', ...           % Click X
               'clickY', ...           % Click Y
               'clickDeg', ...         % Click degrees relative to butterfly heading
               'clickDist',...         % Dlick Distance
               'clickLeftRight',...    % Click to left or right
               'clickFrontBack',...    % Click to front or back
               'clickRGB_R',...        % Click R (used to indiate hits)
               'clickRGB_G',...        % Click G (used to indicate ROIs)
               'clickRGB_B',...        % Click B (used to indicate ROIs)
               'clickTime', ...        % Time of click
               'clickFrame',...        % Frame of click
               'clickHit', ...         % Correct (1/0)
               'butterflyX',...        % Butterfly X
               'butterflyY',...        % Butterfly Y
               'butterflyDeg',...      % Butterfly Heading Degrees
               'stimuliDuration', ...  % Measured trial d9uration (s)
...
lines = {sprintf('On average you hit %.1f%% of butterflies', ...
                 100 * nHits / (nPaths*nRep))};
sizes = {50}; % Initialize as a cell array
colors = {[1*(1-(nHits / (nPaths*nRep))),1*(nHits /(nPaths*nRep))^0.5,0]}; % Initialize as a cell array
xCoords = {0.53}; % Initialize as a cell array
yCoords = {0.6}; % Initialize as a cell array

f_game_UI (window,windowRect,game,9,lines,sizes,colors,xCoords,yCoords); 
f_input_waitForClickRelease;
