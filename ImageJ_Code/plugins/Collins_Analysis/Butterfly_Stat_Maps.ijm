...
if(abs(mn)>abs(mx)) mx=mn;

run("Divide...","value=mx");
run("Multiply...","value=30");
run("Set Min And Max", "min=-15 max=15");
setBatchMode("show");
...
selectImage("Ang");
setSlice(1); run("Square","slice"); setPasteMode("Add");
run("Paste");
...
setBatchMode("show");
 ...
 rename("Orientation");
selectImage("E"); rename("Contrast");
selectImage("P"); rename("Periodicity");
selectImage("DR"); rename("Stripe-Spot");
...
v=getValue("Mean");
run("Subtract...","value=v");
run("Abs"); run("Copy"); setPasteMode("Min");
close();
selectImage("2");
run("Paste");
run("Copy"); setPasteMode("Min");
close();
