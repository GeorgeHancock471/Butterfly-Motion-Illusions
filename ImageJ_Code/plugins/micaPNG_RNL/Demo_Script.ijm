...
verticalityMeasures= newArray(4);
...
tS = channels[i] + "_" + scales[s] + "_" + a*45 + "_contrast";
...
//Mean
tS = channels[i] + "_" + scales[s] +"_contrastAvg";
TitlesArray=Array.concat(TitlesArray,tS);
...
TitlesArray=Array.concat(TitlesArray,tS);
MeasuresArray=Array.concat(MeasuresArray,max/mean);

directionalityMeasures[s] = max/mean;

//Verticality
tS = channels[i] + "_" + scales[s] +"_verticality";
TitlesArray=Array.concat(TitlesArray,tS);
MeasuresArray=Array.concat(MeasuresArray,angleMeasures[0]-angleMeasures[2]);

verticalityMeasures[s] = angleMeasures[0]-angleMeasures[2];
...
Array.show(contrastMeasures);


//Mean Vert
Array.getStatistics(verticalityMeasures, min, max, mean, stdDev);
...
d = imageDIR+FolderID+"_Results.csv";
if(File.exists(d)){
new=0;
}else{
new =1;
